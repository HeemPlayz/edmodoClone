<?php 
   ...
   if(isset($_POST['hashtag'])){
       $hashtag = $getFromUserClass->checkInput($_POST['hashtag']);
       $mention = $getFromUserClass->checkInput($_POST['hashtag']);

       if(substr($hashtag,0,1)==='#'){
            $trend = str_replace('#','',$hashtag);
            $trends = $getFromTweetClass->getTrendByHash($trend);
       ...
       if(substr($mention,0,1)==='@'){
        ...
        $mentions = $getFromTweetClass->getMention($mention);
