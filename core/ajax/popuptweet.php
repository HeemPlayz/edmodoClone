<?php 
    ...
    if(isset($_POST['showpopup']) && !empty ($_POST['showpopup'])){
        ...
        $tweet = $getFromTweetClass->getPopupTweet($tweetID);
        $likes = $getFromTweetClass->likes($user_id,$tweetID);
        $retweet = $getFromTweetClass->checkRetweet($tweetID,$user_id);
	      $user = $getFromUserClass->userData($user_id);
				$comments = $getFromTweetClass->comments($tweetID);
		?>
...
<div class="wrap4">
	...
	<div class="tweet-show-popup-inner">
		...
		<div class="tweet-show-popup-tweet-wrap">
			...
			<?php echo $getFromTweetClass->getTweetLinks($tweet->status );?>
			</div>
			<div class="tweet-show-popup-tweet-ifram">
			<?php if(!empty($tweet->tweetImage)){?>
			<img src="<?php echo BASE_URL.$tweet->tweetImage ;?>"/> <?php } ?>
			</div>
		</div>
		<div class="tweet-show-popup-footer-wrap">
			...
			<div class="tweet-show-popup-footer-menu">
				...
				?>
					...
					<li><button type="button"><i class="fa fa-thumbs-o-up" aria-hidden="true"></i><span class="likesCount"><?php echo $tweet->likesCount; ?></span></button></button></li>
			<?php } ?>
						</ul>	
					</li>
				</ul>
