<?php
    ...
    if(isset($_POST['comment']) && !empty($_POST['comment'])){
        $comment = $getFromTweetClass->checkInput($_POST['comment']);
        $user_id = $_SESSION['user_id'];
        $tweetID = $_POST['tweet_id'];

        if(!empty($comment)){
            $getFromUserClass->create('comments',array('comment'=>$comment, 'commentOn'=>$tweetID,'commentBy'=>$user_id,'commentAt'=>date('Y-m-d H:i:s')));
