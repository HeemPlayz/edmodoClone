<?php 
    ...
    $user = $getFromUserClass->userData($user_id);
		...
		if(isset($_POST['screenName'])){
				if(!empty($_POST['screenName'])){
					$screenName = $getFromUserClass->checkInput($_POST['screenName']);
					$profileBio = $getFromUserClass->checkInput($_POST['bio']);
					$country = $getFromUserClass->checkInput($_POST['country']);
					$website = $getFromUserClass->checkInput($_POST['website']);
		...
		if(isset($_FILES['profileImage'])){
				if(!empty($_FILES['profileImage']['name'][0])){
					$fileRoot= $getFromUserClass->uploadImage($_FILES['profileImage']);
					$getFromUserClass->update('users',$user_id,array('profileImage'=>$fileRoot));
				}
		}
	...
	if(isset($_FILES['profileCover'])){
		if(!empty($_FILES['profileCover']['name'][0])){
			$fileRoot= $getFromUserClass->uploadImage($_FILES['profileCover']);
			$getFromUserClass->update('users',$user_id,array('profileCover'=>$fileRoot));
			header('Location:'.$user->username);
		}
}
?>
		...
		<ul>
			...
			<li>
				<a href="#">
					...
					<div class="n-bottom">
					<?php $getFromTweetClass->countLikes($user_id);?>
					</div>
				</a>
			</li>
...
<div class="in-center">
	<div class="in-center-wrap">	
		<?php 
				$tweets = $getFromTweetClass->getUserTweets($user_id);
							...
							<div class="t-show-banner">
							 ...
							 <div class="t-s-head-content">
									 <div class="t-h-c-name">
											 ...
											 <span>'.$getFromUserClass->timeAgo($retweet['postedOn']).'</span>
									 </div>
									 <div class="t-h-c-dis">
					 ...
					 <div class="t-s-b-inner">
							...
							<div class="t-show-footer">
									<div class="t-s-f-right">
											<ul> 
											'.(($getFromUserClass->loggedIn()=== true) ? '
													...
													<li><button><a href="#"><i style="color:rgb(0, 132, 255);" class="fa fa-thumbs-o-up" aria-hidden="true">
